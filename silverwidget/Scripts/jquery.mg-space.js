...
;(function ( $, window, document, undefined ) {
    ...
    var mgSpace = "mgSpace",
        defaults = {
            ...
            targetPadding: 160, // Padding top/bottom inside target gets divided by 2
        ...
        rowController: function (element, event) {
            ...
            if ((event.data && event.data.close) || $rowItem.hasClass(_.stripDot(_.options.row)+'-open')) {
                ...
                if ($(_.options.row+'-open[data-section="' + itemSection + '"][data-row="' + itemRow + '"]').length) {
                    ...
                    _.closeTarget(400);
                    $('.mg-space').slideToggle(600, function () {
                        _.openRow($rowItem);
                    });
                } else {
                    _.openRow($rowItem);
        ...
        openSpace: function (element) {           
            ...
            $('.mg-space[data-section="' + itemSection + '"][data-row="' + itemRow + '"]').css({
                ...
                if (_.options.useIndicator) {
                    $('.mg-indicator').css({
                        left: itemPosition.left + parseInt($(element).css('padding-left')) + $(element).width()/5 - 75,
                    });
                    $('.mg-indicator').animate({top:-25}, 200);
                }
            });                        
        },
